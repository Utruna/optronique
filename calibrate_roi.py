#!/usr/bin/env python3
"""Calibrage ROI en direct: fenêtre live + ajustement au clavier."""
...
def save_config_values(left, top):
    if not CONFIG_PATH.exists():
        print("❌ config_hardware.py introuvable.")
        return False
    content = CONFIG_PATH.read_text(encoding="utf-8")
    ...
    if n1 == 0 or n2 == 0:
        print("⚠️ Les clés n'ont pas été trouvées. Ajoute-les manuellement.")
        return False
    CONFIG_PATH.write_text(content, encoding="utf-8")
    print("✅ config_hardware.py mis à jour.")
    return True
...
def main():
    print("=== Calibrage ROI (fenêtre live) ===")
    fov = read_int("Taille du FOV (px)", 416)
    ...
    print("\nContrôles:")
    print("  Flèches / WASD : déplacer la ROI")
    print("  +/- : changer le pas (actuel: 5px)")
    print("  O : sauvegarder dans config_hardware.py")
    ...
    while True:
        ...
        cv2.imshow("Calibrage ROI", frame)
        key = cv2.waitKey(1) & 0xFF

        if key in (27, ord("q")):
            break
        elif key in (81, ord("a")):  # gauche
            left -= step
        elif key in (83, ord("d")):  # droite
            left += step
        elif key in (82, ord("w")):  # haut
            top -= step
        elif key in (84, ord("s")):  # bas
            top += step
        elif key in (ord("+"), ord("=")):
            step = min(step + 1, 50)
        ...
        # Petit repos pour éviter de saturer
        time.sleep(0.001)
